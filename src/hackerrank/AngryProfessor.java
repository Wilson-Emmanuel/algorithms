...
public class AngryProfessor {
    public static void main(String[] args) {
        ...
        for(int i=0; i<cases; i++){
            ...
            int n = Integer.parseInt(first[0]);//Number of students in the class
            int k= Integer.parseInt(first[1]);//Cancellation threshold
            int counter =0;
            String[] studs = sc.nextLine().trim().split(" ");
            for(int j=0; j<studs.length; j++){
                int time = Integer.parseInt(studs[j]);
                if(time <=0){
                    counter++;
                }
            }
